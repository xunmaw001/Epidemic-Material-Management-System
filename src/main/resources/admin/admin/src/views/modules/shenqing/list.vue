<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="shenqingYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.shenqingYesnoTypes" placeholder="请选择报名状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.shenqingYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="shenqingYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="shenqingYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '报名编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.shenqingUuidNumber" placeholder="报名编号" clearable></el-input>
                     </el-form-item>
                                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '报名状态' : ''">
                        <el-select v-model="searchForm.shenqingYesnoTypes" placeholder="请选择报名状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in shenqingYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shenqing','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/emmsys/upload/shenqingMuBan.xls"
                        >批量导入物资申请数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shenqing','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shenqingUploadSuccess"
                                :on-error="shenqingUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shenqing','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shenqing.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shenqingUuidNumber"
                                   header-align="center"
                                   label="报名编号">
                        <template slot-scope="scope">
                            {{scope.row.shenqingUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shenqingText"
                                      ...
                                      label="理由">
                        <template slot-scope="scope">
                            <span v-if="scope.row.shenqingText != null &&scope.row.shenqingText.length>10">
                                {{scope.row.shenqingText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.shenqingText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shenqingShuliang"
                                      header-align="center"
                                      label="物资申请数量">
                        <template slot-scope="scope">
                            {{scope.row.shenqingShuliang}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shenqingYesnoTypes"
                                      header-align="center"
                                      label="报名状态">
                        <template slot-scope="scope">
                            {{scope.row.shenqingYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shenqingYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.shenqingYesnoText != null &&scope.row.shenqingYesnoText.length>10">
                                {{scope.row.shenqingYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.shenqingYesnoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shenqingShenheTime"
                                   header-align="center"
                                   label="审核时间">
                        <template slot-scope="scope">
                            {{scope.row.shenqingShenheTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('shenqing','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shenqing','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shenqing','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('shenqing','审核') && scope.row.shenqingYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              shenqingYesnoTypesSelectSearch : [],
            form:{
                id : null,
                shenqingUuidNumber : null,
                wuziId : null,
                yonghuId : null,
                shenqingText : null,
                insertTime : null,
                shenqingShuliang : null,
                shenqingYesnoTypes : null,
                shenqingYesnoText : null,
                shenqingShenheTime : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            shenqingYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '报名编号': "shenqingUuidNumber",
                     '理由': "shenqingText",
                     '物资申请时间': "insertTime",
                     '物资申请数量': "shenqingShuliang",
                     '报名状态': "shenqingYesnoValue",
                     '审核回复': "shenqingYesnoText",
                     '审核时间': "shenqingShenheTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shenqing',//当前表表名,
                        sumColum : 'shenqing_shuliang', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shenqing_name',//分组字符串字段
                        types : 'shenqing_yesno_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.shenqingUuidNumber!= '' && this.searchForm.shenqingUuidNumber!= undefined) {
                    params['shenqingUuidNumber'] = '%' + this.searchForm.shenqingUuidNumber + '%'
                }
                                                 
                if (this.searchForm.shenqingYesnoTypes!= '' && this.searchForm.shenqingYesnoTypes!= undefined) {
                    params['shenqingYesnoTypes'] = this.searchForm.shenqingYesnoTypes
                }
                        
                params['shenqingDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=shenqing_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.shenqingYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            shenqingUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shenqing/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            shenqingUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.shenqingYesnoText = null;
                _this.form.shenqingYesnoTypes = "请选择报名状态";
                _this.shenqingYesnoTypesVisible = true;
            },

            shenqingYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.shenqingYesnoTypes == "请选择报名状态"){
                    _this.$message.error("请选择报名状态");return false;
                }
                if(_this.form.shenqingYesnoText == null || _this.form.shenqingYesnoText == ""){
                ...
                this.$http({
                    ...
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.shenqingYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
