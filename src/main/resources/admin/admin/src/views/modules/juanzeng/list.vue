<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="juanzengYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.juanzengYesnoTypes" placeholder="请选择报名状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.juanzengYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="juanzengYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="juanzengYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '报名编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.juanzengUuidNumber" placeholder="报名编号" clearable></el-input>
                     </el-form-item>
                                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '报名状态' : ''">
                        <el-select v-model="searchForm.juanzengYesnoTypes" placeholder="请选择报名状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in juanzengYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('juanzeng','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/emmsys/upload/juanzengMuBan.xls"
                        >批量导入物资捐赠数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('juanzeng','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="juanzengUploadSuccess"
                                :on-error="juanzengUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('juanzeng','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "juanzeng.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="juanzengUuidNumber"
                                   header-align="center"
                                   label="报名编号">
                        <template slot-scope="scope">
                            {{scope.row.juanzengUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="juanzengText"
                                      ...
                                      label="理由">
                        <template slot-scope="scope">
                            <span v-if="scope.row.juanzengText != null &&scope.row.juanzengText.length>10">
                                {{scope.row.juanzengText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.juanzengText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="juanzengYesnoTypes"
                                      header-align="center"
                                      label="报名状态">
                        <template slot-scope="scope">
                            {{scope.row.juanzengYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="juanzengYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.juanzengYesnoText != null &&scope.row.juanzengYesnoText.length>10">
                                {{scope.row.juanzengYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.juanzengYesnoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="juanzengShenheTime"
                                   header-align="center"
                                   label="审核时间">
                        <template slot-scope="scope">
                            {{scope.row.juanzengShenheTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('juanzeng','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('juanzeng','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('juanzeng','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('juanzeng','审核') && scope.row.juanzengYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              juanzengYesnoTypesSelectSearch : [],
            form:{
                id : null,
                juanzengUuidNumber : null,
                wuziId : null,
                yonghuId : null,
                juanzengText : null,
                insertTime : null,
                juanzengShuliang : null,
                juanzengYesnoTypes : null,
                juanzengYesnoText : null,
                juanzengShenheTime : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            juanzengYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '报名编号': "juanzengUuidNumber",
                     '理由': "juanzengText",
                     '物资捐赠时间': "insertTime",
                     '物资捐赠数量': "juanzengShuliang",
                     '报名状态': "juanzengYesnoValue",
                     '审核回复': "juanzengYesnoText",
                     '审核时间': "juanzengShenheTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'juanzeng',//当前表表名,
                        sumColum : 'juanzeng_shuliang', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'juanzeng_name',//分组字符串字段
                        types : 'juanzeng_yesno_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.juanzengUuidNumber!= '' && this.searchForm.juanzengUuidNumber!= undefined) {
                    params['juanzengUuidNumber'] = '%' + this.searchForm.juanzengUuidNumber + '%'
                }
                                                 
                if (this.searchForm.juanzengYesnoTypes!= '' && this.searchForm.juanzengYesnoTypes!= undefined) {
                    params['juanzengYesnoTypes'] = this.searchForm.juanzengYesnoTypes
                }
                        
                params['juanzengDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=juanzeng_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.juanzengYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            juanzengUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "juanzeng/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            juanzengUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.juanzengYesnoText = null;
                _this.form.juanzengYesnoTypes = "请选择报名状态";
                _this.juanzengYesnoTypesVisible = true;
            },

            juanzengYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.juanzengYesnoTypes == "请选择报名状态"){
                    _this.$message.error("请选择报名状态");return false;
                }
                if(_this.form.juanzengYesnoText == null || _this.form.juanzengYesnoText == ""){
                ...
                this.$http({
                    ...
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.juanzengYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
